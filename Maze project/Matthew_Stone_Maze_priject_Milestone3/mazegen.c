                 ...
                 int printAlgorithmSteps)            // [TRUE | FALSE]
{
  ...
  if(printAlgorithmSteps)
  {
    printf("Assigning entrance and exit...");
  }
...
void mazePrint()
{
  ...
  //printf("pixelWidth = %d, pixelHeight = %d\n", pixelWidth, pixelHeight);
  ...
  printf("Writing maze to .BMP, please wait...");
  ...
  char file_in[100];
  char solved_holder[100];
  for(i = 0; i < y_size; i++)
  {
    for(j = 0; j < x_size; j++)
    {
      strcpy(solved_holder, "s_");
      strcpy(file_in, "");
      //printf("%c\n", (unsigned char)*(maze + i * x_size + j));
      //printf("length of file_in = %ld\n", strlen(file_in));
      //printf("length of solved_holder = %ld\n", strlen(solved_holder));
      switch((unsigned char)*(maze + i * x_size + j))
      {
        case 219:
          strcpy(file_in, "no_direction.bmp");//should never be this case, just here in case
          break;
        case 208:
          strcpy(file_in, "north.bmp");
          if(*(maze_cells + i * x_size + j) & VISITED)
          {
            strcat(solved_holder, file_in);
            strcpy(file_in, solved_holder);
          }
          break;
        case 198:
          ...
          if(*(maze_cells + i * x_size + j) & VISITED)
          {
            strcat(solved_holder, file_in);//strcpy(file_in, strcat(solved_holder, file_in));
            strcpy(file_in, solved_holder);
          }
          break;
        case 200:
        ...
        case 201:
          ...
          break;
        case 204:
          strcpy(file_in, "south_east_north.bmp");
          if(*(maze_cells + i * x_size + j) & VISITED)
          {
            strcat(solved_holder, file_in);
            strcpy(file_in, solved_holder);
        ...
        case 202:
          ...
          if(*(maze_cells + i * x_size + j) & VISITED)
          {
            strcat(solved_holder, file_in);//solved_holder = "s_", file_in = "west_north_east.bmp"
            strcpy(file_in, solved_holder);
          }
          break;
        case 187:
      ...
      for(y = imageHeight-1; y>=0; y--)
      {
        ...
        for (x=0; x<imageWidth; x++)
        {
          ...
          //printf("Writing RGB at (%3d, %3d)\n", (j*8)+x, (i*8)+y);
          setRGB(maze_img, (j*imageWidth)+x, (i*imageHeight)+y, rowSize, pixelHeight, r, g, b);
        }
      }
    }
  }
